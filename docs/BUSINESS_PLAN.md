...
flowchart LR
    subgraph TAM["Total Addressable Market"]
        M1[Muslim consumer spending]
        M2[Local services & SMEs]
        M3[Professional services]
        M4[Islamic institutions]
    end

    subgraph SAM["Serviceable Available Market"]
        S1[Digitally discoverable services]
        S2[Bangladesh + similar markets]
    end

    subgraph SOM["Serviceable Obtainable Market Year 3"]
        O1[100K vendors]
        O2[1M MAU]
    end
...
flowchart TB
    subgraph KeyPartners["Key Partners"]
        P1[Mosques & madaris]
        P2[Islamic banks / Takaful]
        P3[NGOs & waqf boards]
        P4[Payment gateways]
        P5[Local business associations]
    end

    subgraph KeyActivities["Key Activities"]
        A1[Vendor onboarding]
        A2[Trust verification]
        A3[Search & ranking]
        A4[Ecosystem marketing]
        A5[Community trust building]
    end
    ...
    subgraph CustomerSegments["Customer Segments"]
        C1[Muslim consumers]
        C2[SMEs & freelancers]
        C3[Islamic service providers]
        C4[Institutions]
    end

    subgraph Revenue["Revenue Streams"]
        R1[Vendor subscriptions]
        R2[Featured & sponsored]
        R3[Lead fees]
        R4[Transaction fees]
        R5[Islamic finance referrals]
        R6[Enterprise API]
    end

    KeyPartners --> KeyActivities
    KeyActivities --> ValueProp
    ValueProp --> CustomerSegments
    CustomerSegments --> Revenue
```
...
```mermaid
flowchart TD
    subgraph Phase1["Phase 1 : Bangladesh MVP"]
        RS1[Vendor Subscription SaaS]
        RS2[Featured Listings]
        RS3[Lead Generation Fees]
    end

    subgraph Phase2["Phase 2 : Transactions"]
        RS4[Booking Fee]
        RS5[Payment Processing Margin]
        RS6[Sponsored Search CPC]
    end

    subgraph Phase3["Phase 3 : Ecosystem"]
        RS7[Islamic Finance Referrals]
        RS8[Waqf / Charity Platform Fee]
        RS9[Enterprise & API]
        RS10[Analytics & Insights SaaS]
    end
...
```mermaid
flowchart LR
    CAC[Vendor CAC<br/>~500 BDT] --> Onboard[Onboarded Vendor]
    Onboard --> Free[Free plan]
    Free --> Convert{Converts in 90d?}
    Convert -->|20% target| Pro[Pro 999 BDT/mo]
    Convert -->|5% target| Biz[Business 2499 BDT/mo]
    Pro --> LTV[LTV 12 mo = 12,000 BDT]
    Biz --> LTV2[LTV 12 mo = 30,000 BDT]
```
...
```mermaid
flowchart TD
    Demand[Publish real demand] --> Curiosity[Vendors ask where leads come from]
    Curiosity --> Signup[Vendors sign up]
    Signup --> Supply[Growing supply]
    Supply --> Results[Users find solutions]
    Results --> Stories[Success stories]
    Stories --> Demand
```
...
```mermaid
flowchart TD
    A[See demand post or SEO result] --> B{Click destination}
    B --> C[Land on category or search page]
    C --> D[Browse vendors]
    D --> E[Request quote / call]
    E --> F{Account?}
    F -->|No| G[Quick OTP signup]
    F -->|Yes| H[Lead submitted]
    G --> H
    H --> I[Vendor responds]
    I --> J[User reviews]
    J --> K[User returns + refers friends]
    K --> A
```
...
```mermaid
flowchart TD
    A[Sees demand or competitor on Taqdimah] --> B[Visits Taqdimah.com/register/vendor]
    B --> C[Creates profile]
    C --> D[Submits verification]
    D --> E[Approved in 48h]
...
```mermaid
flowchart LR
    Boot[Bootstrap MVP] --> iDEA[iDEA Grant BDT 10L]
    iDEA --> Traction[500 vendors + 5K MAU]
    Traction --> SBL[Startup Bangladesh Seed]
    SBL --> Angels[Bangladesh Angels]
    Angels --> Regional[Anchorless / SEA funds]
    Regional --> Global[Islamic VC / impact funds]
```
