...
```mermaid
flowchart TD
    A[User opens Taqdimah] --> B[Types natural language need]
    B --> C[Search + category + location]
    C --> D[Results ranked by trust score]
    D --> E{User action}
    E -->|View profile| F[Vendor profile + reviews]
    E -->|Request quote| G[Lead form submitted]
    G --> H[Vendor notified]
    H --> I[Vendor responds]
    I --> J[Transaction off-platform or in-app Phase 2]
    J --> K[User leaves review]
    K --> D
```
...
```mermaid
flowchart TD
    A[Vendor hears demand post] --> B[Signs up on Taqdimah]
    B --> C[Creates business profile]
    C --> D[Submits verification docs]
    D --> E{Admin review}
    E -->|Approved| F[Profile goes live]
    E -->|Rejected| C
    F --> G[Receives lead notifications]
    G --> H[Responds to customer]
    H --> I[Optional: upgrade to premium]
    I --> J[Better placement + analytics]
```
...
```mermaid
flowchart LR
    Need[Real customer need posted] --> Social[Social / community channels]
    Social --> Vendors[Vendors see demand]
    Vendors --> Join[Join Taqdimah]
    Join --> Supply[More supply]
    Supply --> Users[Users find answers]
    Users --> Need
```
